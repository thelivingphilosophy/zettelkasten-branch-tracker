import { Plugin, ItemView, PluginSettingTab, Setting, TFile, WorkspaceLeaf, Notice } from 'obsidian';
import { ZettelNode, ZettelEdge, ZettelNetwork, PluginSettings, MousePosition, PanOffset } from './types';
...
export default class ZettelkastenBranchTracker extends Plugin {
    ...
    async onload(): Promise<void> {
        ...
        this.registerView(
            ...
            (leaf: WorkspaceLeaf) => new ZettelkastenBranchView(leaf, this)
        );
    ...
    parseZettelId(filename: string): string | null {
        const match = filename.match(/^([0-9]+(?:[a-z]+[0-9]*|\.[0-9]+)*(?:[a-z]+[0-9]*)*)(?:\s|$)/i);
        return match ? match[1] : null;
    }

    findParentId(zettelId: string): string | null {
    ...
    getZettelNotes(): Map<string, TFile> {
        const zettelNotes = new Map<string, TFile>();
    ...
    buildZettelNetwork(currentZettelId: string, maxDepth: number = 2, maxBranches: number = 5): ZettelNetwork | null {
    ...
    calculateNodeDepth(nodeId: string, zettelNotes: Map<string, TFile>): number {
        let totalSubnotes = 0;
    ...
    addDepthDataToNodes(network: ZettelNetwork, zettelNotes: Map<string, TFile>, currentZettelId: string): void {
    ...
    addGreatGrandparentTier(currentZettelId: string, zettelNotes: Map<string, TFile>, nodes: Map<string, ZettelNode>, edges: ZettelEdge[], maxBranches: number): void {
        ...
        const greatGrandparentNode: ZettelNode = {
        ...
        if (closestGreatAuntsUncles.length > 0) {
            closestGreatAuntsUncles.forEach((auntUncleId, index) => {
                ...
                const auntUncleNode: ZettelNode = {
                    id: auntUncleId,
    ...
    addGrandparentTier(currentZettelId: string, zettelNotes: Map<string, TFile>, nodes: Map<string, ZettelNode>, edges: ZettelEdge[], maxBranches: number): void {
        const grandparentId = this.findGrandparentId(currentZettelId);
        if (!grandparentId || !zettelNotes.has(grandparentId)) return;

        const grandparentFile = zettelNotes.get(grandparentId)!;
        const grandparentNode: ZettelNode = {
            id: grandparentId,
    ...
    addParentTier(currentZettelId: string, zettelNotes: Map<string, TFile>, nodes: Map<string, ZettelNode>, edges: ZettelEdge[], maxBranches: number): void {
        const parentId = this.findParentId(currentZettelId);
    ...
    addCurrentNoteTier(currentZettelId: string, zettelNotes: Map<string, TFile>, nodes: Map<string, ZettelNode>, edges: ZettelEdge[], maxBranches: number): void {
        const currentFile = zettelNotes.get(currentZettelId)!;
        const currentNode: ZettelNode = {
            id: currentZettelId,
        ...
        linearContinuations.slice(0, Math.max(0, maxBranches - 1)).forEach((continuationId, index) => {
            ...
            const continuationNode: ZettelNode = {
                id: continuationId,
    ...
    addChildrenTier(currentZettelId: string, zettelNotes: Map<string, TFile>, nodes: Map<string, ZettelNode>, edges: ZettelEdge[], maxBranches: number): void {
    ...
    findGrandparentId(zettelId: string): string | null {
        const parentId = this.findParentId(zettelId);
        if (!parentId) return null;
        return this.findParentId(parentId);
    }

    findPreviousNoteInSequence(zettelId: string, zettelNotes: Map<string, TFile>): string | null {
    ...
    isLinearContinuation(baseId: string, otherId: string): boolean {
    ...
    getClosestSiblings(currentId: string, allSiblings: string[], maxBranches: number): string[] {
    ...
    findLinearContinuations(baseId: string, zettelNotes: Map<string, TFile>): string[] {
    ...
    findTrueChildren(baseId: string, zettelNotes: Map<string, TFile>): string[] {
    ...
    findChildIds(parentId: string, zettelNotes: Map<string, TFile>): string[] {
    ...
    compareZettelIds(a: string, b: string): number {
    ...
    // Navigation methods
    async navigateToSibling(direction: 'next' | 'previous'): Promise<void> {
    ...
    async navigateToFirstChild(): Promise<void> {
        const activeFile = this.app.workspace.getActiveFile();
        if (!activeFile) {
            new Notice('No active file');
            return;
    ...
    async navigateToContinuation(direction: 'next' | 'previous'): Promise<void> {
        const activeFile = this.app.workspace.getActiveFile();
        if (!activeFile) {
            new Notice('No active file');
            return;
...
class ZettelkastenBranchView extends ItemView {
    plugin: ZettelkastenBranchTracker;
    currentDepth: number;
    currentMaxBranches: number;
    zoom: number;
    panOffset: PanOffset;

    showSubnoteCounts: boolean;
    autoUpdate: boolean;

    network: ZettelNetwork | null;

    // UI elements
    canvas!: HTMLCanvasElement;
    ctx!: CanvasRenderingContext2D;
    depthSlider!: HTMLInputElement;
    depthLabel!: HTMLSpanElement;
    branchSlider!: HTMLInputElement;
    branchLabel!: HTMLSpanElement;
    forceSlider!: HTMLInputElement;
    forceLabel!: HTMLSpanElement;
    countCheckbox!: HTMLInputElement;
    autoUpdateCheckbox!: HTMLInputElement;
    resizeObserver!: ResizeObserver;
    updateCanvasContainerSize!: () => void;

    mousePos: MousePosition;
    hoveredNode: ZettelNode | null;
    isDragging: boolean;
    dragStartPos: MousePosition;
    lastPanOffset: PanOffset;

    nodeScales: Map<string, number>;
    targetScales: Map<string, number>;
    textOffsets: Map<string, number>;
    targetTextOffsets: Map<string, number>;
    animationSpeed: number;
    animationTime: number; // For pulsing animation

    animationId: number | null;
    needsRender: boolean;

    constructor(leaf: WorkspaceLeaf, plugin: ZettelkastenBranchTracker) {
        super(leaf);
        this.plugin = plugin;
    ...
    async onOpen(): Promise<void> {
        ...
        this.depthSlider.addEventListener('input', (e) => {
            this.currentDepth = parseInt((e.target as HTMLInputElement).value);
            this.depthLabel.textContent = String(this.currentDepth);
            this.updateView();
        });

        this.branchSlider.addEventListener('input', (e) => {
            this.currentMaxBranches = parseInt((e.target as HTMLInputElement).value);
        ...
        this.countCheckbox.addEventListener('change', (e) => {
            this.showSubnoteCounts = (e.target as HTMLInputElement).checked;
            this.needsRender = true;
        });
        ...
        this.canvas = canvasContainer.createEl('canvas') as HTMLCanvasElement;
    ...
    updateNodeAnimations(): void {
        if (!this.network) return;

        let hasAnimations = false;
        
        // Update pulsing animation time
    ...
    isDarkTheme(): boolean {
        // Check if Obsidian is in dark mode by examining the body class
    ...
    getComplementaryHoverColor(originalColor: string): string {
    ...
    renderNetwork(network: ZettelNetwork): void {
        this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
    ...
    drawNode(node: ZettelNode, centerX: number, centerY: number, spacingMultiplier: number): void {
        ...
        if (node.isCenter) {
            // Use high contrast colors for center node
        ...
        // Draw the main node
        if (node.isCenter) {
            // Draw square for center node with pulsing glow
            const pulseIntensity = (Math.sin(this.animationTime) + 1) / 2; // 0 to 1
            const squareSize = radius * 1.4;
            const glowSize = squareSize + (pulseIntensity * 12) + 8; // Larger glow area
            
            // Draw multiple glow layers for stronger effect
            const glowColor = this.isDarkTheme() ? '#FFFFFF' : '#000000';
            const glowOpacity = 0.3 + (pulseIntensity * 0.3); // Pulse opacity too
            ...
            this.ctx.fillStyle = fillColor; // Now white/black for high contrast
            this.ctx.fill();
            ...
            // No border needed - the white/black square is the contrast element
        } else {
            // Draw circle for other nodes with subtle glow
            const subtleGlowSize = radius + 4; // Small glow radius
            
            // Draw subtle glow effect
            const subtleGradient = this.ctx.createRadialGradient(x, y, radius, x, y, subtleGlowSize);
            subtleGradient.addColorStop(0, fillColor + '60'); // Start with some opacity at node edge
            subtleGradient.addColorStop(0.5, fillColor + '40'); // 25% opacity in middle
            subtleGradient.addColorStop(1, fillColor + '00'); // Transparent at edge
        ...
        // Add a subtle border for nodes with significant content (but not center nodes)
        if (!node.isCenter && [1, -2, 0].includes(node.level) && node.depthScore !== undefined && node.depthScore > 0.3) {
            this.ctx.beginPath();
            this.ctx.arc(x, y, radius, 0, 2 * Math.PI);
    ...
    drawLabel(node: ZettelNode, centerX: number, centerY: number, spacingMultiplier: number): void {
        const x = centerX + (node.position.x * this.zoom * spacingMultiplier);
        const nodeY = centerY + (node.position.y * this.zoom * spacingMultiplier);
        ...
        if (node.isCenter) {
            actualRadius = baseRadius * 1.4; // Square size multiplier
        }

        // Position text below the actual node size with extra padding for center nodes
        const textPadding = node.isCenter 
            ? Math.max(12, 18 * this.zoom) // Extra space for center node with glow
    ...
    drawSubnoteCount(node: ZettelNode, centerX: number, centerY: number, spacingMultiplier: number): void {
...
class ZettelkastenSettingTab extends PluginSettingTab {
    ...
    constructor(app: any, plugin: ZettelkastenBranchTracker) {
        super(app, plugin);
        this.plugin = plugin;
    }
